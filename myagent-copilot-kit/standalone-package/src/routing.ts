...
export type RouteHint = "auto" | "local" | "copilot";
...
export function decideRoute(params: {
  ...
  routeHint?: string;
  policyRouteHintOverride?: RouteHint;
  hasScope?: boolean;
  ...
  const routeHint = normalizeRouteHint(params.policyRouteHintOverride ?? params.routeHint);
  ...
  if (routeHint === "local") {
    return {
      route: "local",
      reason: "routeHint forced local route.",
      sensitivity,
      dlpStatus: params.dlp.status,
    };
  }

  if (routeHint === "copilot") {
    return {
      route: "copilot",
      reason: "routeHint forced Copilot route.",
      sensitivity,
      dlpStatus: params.dlp.status,
    };
  }
  ...
  if (params.hasScope) {
    return {
      route: "local",
      reason: "Scoped requests require local RAG.",
      sensitivity,
      dlpStatus: params.dlp.status,
    };
  }

  if (params.ragRequested) {
    return {
      route: "local",
      reason: "Request looks like document or RAG work.",
      sensitivity,
      dlpStatus: params.dlp.status,
    };
  }
