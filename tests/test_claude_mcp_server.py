...
def test_create_claude_mcp_server_exposes_read_only_search_fetch_and_recent_listing(
    tmp_path: Path,
):
...
def test_claude_search_treats_explicit_recent_note_query_as_recent_browse(tmp_path: Path):
    ...
    for offset in range(30):
        store.save(
            MemoryCreate(
                ...
                title=f"Claude Specialist Write Check {offset:02d}",
    ...
    business = store.save(
        MemoryCreate(
            memory_type="decision",
            title="Operations follow-up note",
    ...
    server = create_claude_mcp_server(store)
    search_tool = server._tool_manager.get_tool("search")
    payload = json.loads(asyncio.run(search_tool.fn("최근 메모 목록")))

    assert payload["results"][0]["id"] == business["id"]


def test_claude_search_keeps_date_only_general_query_searchable(tmp_path: Path):
    if FastMCP is None:  # pragma: no cover
        return
