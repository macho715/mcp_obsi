...
> **[Cost Domain Rule]** Cost analysis uses 
oute_type (journey classification) and WarehouseHandlingProfile (WH handling evidence) as cost drivers.
> Cost domain does NOT own or assign confirmedFlowCode. Cost rates are referenced by 
oute_type + wh_handling_cnt.
...
- __흐름/승인/근거\(정합성\)__
	- 코스트가드 플로우: hvdc:LogisticsRoute ⟶ hvdc:InvoiceAuditStep \(HVDC Logistics Unified v3\.7 내 일부\)\.
...
- __COST\-GUARD 플로우__: hvdc:LogisticsRoute 안에 hvdc:InvoiceAuditStep을 명시해 "어느 단계에서 무슨 규칙으로 걸렸나"를 설명가능하게\.
