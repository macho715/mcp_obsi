...
if [[ ! -f "$OPENCLAW_HOME/scripts/myagent-proxy.mjs" ]]; then
  echo "scripts/myagent-proxy.mjs not found. Set OPENCLAW_HOME to OpenClaw repo root." >&2
  exit 1
fi

if [[ -z "${MYAGENT_PROXY_CORS_ORIGINS:-}" ]]; then
  echo "MYAGENT_PROXY_CORS_ORIGINS is required in standalone public mode." >&2
  exit 1
fi
if [[ -z "${MYAGENT_PROXY_AUTH_TOKEN:-}" ]]; then
  echo "MYAGENT_PROXY_AUTH_TOKEN is required in standalone public mode." >&2
  exit 1
fi
...
echo "Execution mode     : standalone-direct"
echo "Proxy mode         : public"
echo "Host/Port          : $MYAGENT_PROXY_HOST:$MYAGENT_PROXY_PORT"
echo "Allowed origins    : $MYAGENT_PROXY_CORS_ORIGINS"
echo "Launch command     : node --import tsx scripts/myagent-proxy.mjs"
