...
def process_file(file_path):
    ...
    prompt = PROMPT_TEMPLATE.format(slug=slug, raw_text=raw_text[:4000]) # Limit to 4000 chars to avoid context overflow
    
    messages = [
        {"role": "system", "content": "You are a logistics expert. Respond only with the requested Markdown and YAML frontmatter."},
...
def main():
    ...
    print(f"Found {len(target_files)} files. Starting parallel processing using Local LLM (Ollama)...")
