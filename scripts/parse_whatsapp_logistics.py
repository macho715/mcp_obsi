...
try:
    import scripts.ollama_kb  # noqa: F401
except ImportError:
...
CHANNELS = {
    ...
    "DSV Delivery": {
        ...
        "persona": "You are an inland transport and warehouse coordinator for the UAE HVDC project.",  # noqa: E501
    },
    "Project Lightning": {
    ...
    "Jopetwil 71 Group": {
        ...
        "persona": "You are a specialized barge and port operations coordinator for the UAE HVDC project.",  # noqa: E501
    },
    "MIR Logistics": {
    ...
    "SHU Logistics": {
        ...
        "persona": "You are a site logistics and inspection coordinator at the SHU site for the UAE HVDC project.",  # noqa: E501
    },
}
...
PRE_CONSTRAINTS = [r"D-1 16:00 Planning", r"Hold at DSV", r"weather", r"tide", r"fog"]
FALLBACK_KEYWORDS = [
    r"delay",
    r"hold",
    r"cancel",
...
LOG_PATTERN = re.compile(
    r"^(\d{2}/\d{1,2}/\d{1,2})\s+(AM|PM|오전|오후)\s+(\d{1,2}:\d{2})\s+-\s+([^:]+):\s+(.*)$"
)
...
def extract_events(file_path):
    ...
    for ev in events:
        ...
        event_blocks.append({"start": start_time, "end": end_time, "messages": block_msgs})
...
def process_event(block, index, channel_info):
    ...
    # 2. Call LLM for structuring (Skipped - LLM takes too long, delegating to Assistant in parallel)  # noqa: E501
...
def main():
    ...
    for channel_name, info in CHANNELS.items():
        ...
        for file_path in matched_files:
            ...
            # No MAX_TO_PROCESS limit, process everything (Modified: Limited to 1 per channel to test parallel assistant delegation)  # noqa: E501
            MAX_TO_PROCESS = 10
