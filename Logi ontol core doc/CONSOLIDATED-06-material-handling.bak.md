...
|-------------------|-------------------|-----------------|--------|
| **Container Cargo** | route_type: WH_ONLY, WH_MOSB | Port → WH → (MOSB) → Site | Standard containerized items via warehouse |
| **Bulk Cargo** | route_type: MOSB_DIRECT, WH_MOSB | Port → (WH →) MOSB → Site | Large items requiring MOSB consolidation |
| **Transformer/Heavy** | route_type: MOSB_DIRECT, WH_MOSB | Port → MOSB → AGI/DAS | Specialized heavy equipment, offshore delivery |
| **Direct to MIR/SHU** | route_type: DIRECT | Port → Site | Onshore sites, no MOSB leg required |
| **AGI/DAS Materials** | Flow Code 3, 4 (강제) | Port → MOSB → AGI/DAS | **Mandatory MOSB leg** per domain rules |
...
- Destination = AGI OR DAS
  → Original route_type DIRECT/WH_ONLY → Auto-upgrade → MOSB_DIRECT
...
#### Phase A: Import & Customs (shipment_stage: PRE_ARRIVAL → WH_INBOUND)
...
route_type DIRECT (Onshore Site):
...
route_type WH_ONLY (Warehouse Consolidation):
...
#### Phase B: Offshore Transportation (route_type: MOSB_DIRECT / WH_MOSB)
...
route_type WH_MOSB (Warehouse + MOSB):
...
route_type MIXED (Mixed/Incomplete):
...
mh:hasRouteType a owl:DatatypeProperty ;
...
# SHACL Constraint: AGI/DAS MUST have Flow Code >= 3
        ...
        sh:select """
            ...
            WHERE {
                $this mh:hasDestinationSite ?site ;
                      mh:hasRouteType ?routeType .
                ?site mh:siteCode ?siteCode .
                FILTER(?siteCode IN ("AGI", "DAS") && ?routeType NOT IN ("MOSB_DIRECT","WH_MOSB","MIXED"))
            }
        """ ;
    ] .
```
...
mh:cargo/TRANSFORMER-AGI-001 a mh:Cargo ;
    ...
    mh:hasRouteType "MOSB_DIRECT" ;
    mh:hasOriginalRouteType "DIRECT" ;
    mh:hasRouteOverrideReason "AGI offshore - mandatory MOSB leg" ;
    mh:hasRouteDescription "Port → MOSB → AGI (LCT transport)" .
...
SELECT ?site ?routeType (COUNT(?cargo) AS ?count)
WHERE {
    ?cargo a mh:Cargo ;
...
GROUP BY ?site ?routeType
ORDER BY ?site ?routeType
```
...
SELECT ?cargo ?cargoId ?site ?routeType ?compliant
WHERE {
    ?cargo a mh:Cargo ;
           mh:cargoId ?cargoId ;
           mh:hasDestinationSite ?siteObj ;
...
#### 3. MIXED route_type (Incomplete) Materials at MOSB
...
SELECT ?cargoType ?routeType (COUNT(?cargo) AS ?count) (COUNT(?cargo) AS ?count)
WHERE {
    ?cargo a mh:Cargo ;
...
|------------|--------|-------------|---------------------|
| **MOSB Utilization** | 80-90% | (MOSB_DIRECT + WH_MOSB) / Total | Offshore transport efficiency |
| **Direct Shipping (Onshore)** | 30-40% | DIRECT / Total | MIR/SHU direct delivery rate |
| **Warehouse Efficiency** | 50-60% | (WH_ONLY + WH_MOSB) / Total | Consolidation & staging |
| **AGI/DAS Compliance** | 100% | AGI/DAS with Flow Code ≥3 / AGI/DAS Total | Mandatory MOSB leg compliance |
| **MIXED Route Resolution Time** | <3 days | Avg(Site Assignment Date - MOSB Arrival) | Incomplete routing resolution |
...
#### Section 3 (Storage & Inland): route_type DIRECT, WH_ONLY, WH_MOSB
- DSV Indoor/AAA Storage → route_type WH_ONLY, WH_MOSB (warehouse leg)
- Direct MIR/SHU delivery → route_type DIRECT

#### Section 4 (Offshore Marine): route_type MOSB_DIRECT, WH_MOSB
...
#### Section 6 (Transformer): route_type MOSB_DIRECT, WH_MOSB (Heavy Equipment)
- Specialized heavy equipment always via MOSB
- AGI transformer installation projects

#### Section 7 (Bulk Cargo): route_type MOSB_DIRECT, WH_MOSB (Bulk Operations)
...
\```turtle
mh:LogisticsRoute a owl:Class ;
    rdfs:label "LogisticsRoute" ;
    rdfs:comment "Class representing LogisticsRoute" .
...
mh:has_logisticsflowId a owl:DatatypeProperty ;
    rdfs:label "has LogisticsRoute ID" ;
    rdfs:domain mh:LogisticsRoute ;
    rdfs:range xsd:string .
...
mh:LogisticsFlowShape a sh:NodeShape ;
    sh:targetClass mh:LogisticsRoute ;
    sh:property [
        sh:path mh:has_logisticsflowId ;
        sh:minCount 1 ;
        sh:message "LogisticsRoute must have ID"
    ] .
...
\```json
{
  ...
  "@id": "mh:LogisticsRoute-001",
  "@type": "mh:LogisticsRoute",
  "mh:has_logisticsflowId": "MH-001",
  "mh:hasDescription": "Example LogisticsRoute"
}
\```
...
SELECT ?LogisticsRoute ?description WHERE {
    ?LogisticsRoute a mh:LogisticsRoute .
    ?LogisticsRoute mh:hasDescription ?description .
}
LIMIT 10
\```
...
**MOSB Operation Process Example**
